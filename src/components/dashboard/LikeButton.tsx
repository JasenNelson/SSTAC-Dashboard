...
export default function LikeButton({ 
  ...
  const [_popupPosition, _setPopupPosition] = useState<'top' | 'bottom'>('top');
  ...
  return (
    <div className={`relative ${className}`} style={{ overflow: 'visible' }}>
      ...
      {showDetails && likes > 0 && (
        <div 
          id="like-popup"
          className="fixed w-64 bg-white border border-slate-200 rounded-lg shadow-lg z-[9999]"
          style={{
            maxHeight: '300px'
          }}>
          <div className="p-3 border-b border-slate-100">
            <h4 className="font-medium text-slate-900">Liked by</h4>
          </div>
          <div className="max-h-48 overflow-y-auto">
            {likeDetails.map((like) => (
              <div key={like.id} className="flex items-center justify-between p-3 hover:bg-slate-50">
