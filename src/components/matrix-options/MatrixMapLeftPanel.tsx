...
//   - Substance card REMOVED (substance filter moves to right panel).
//   - Medium radio card REMOVED (medium filter moves to right panel as chips).
//   - substanceKey prop DROPPED (no longer needed after above removals).
//     NOTE: Phase 1 will re-add measurementStore reads for the stats engine;
//     the removal here is correct for a clean Phase 0 seam.
//   - MatrixMapStatsShell placeholder added (Phase 1 replaces its internals).
//   - Root div changed from w-80 to w-full h-full to fill variable-width wrapper.
    ...
    <div className="w-full h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {hasSelection ? (
          ...
          <div
            data-testid="matrix-map-left-panel-empty-state"
          ...
          >
            ...
            <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
              Select a sample on the map to review composition, media, and
              selection export controls. Use identify mode to inspect WMS
              features without adding samples to the selection.
            </p>
          </div>
        )}
      </div>
