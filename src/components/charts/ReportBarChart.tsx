...
export default function ReportBarChart({
  ...
  const ariaLabel = useMemo(() => {
    ...
    if (parts.length === 0) {
      parts.push('Horizontal bar chart');
    }
    parts.push(`Displaying ${sortedData.length} data points`);
    return parts.join('. ');
      ...
      return (
        ...
        >
          ...
          <p style={{ color: tooltipText }}>
            Value: <span style={{ fontWeight: 600 }}>{data.value}</span>
          </p>
        </div>
      );
    }
              ...
              label={
                yAxisLabel
                  ? {
                      ...
                      angle: -90,
                      position: 'insideLeft',
                      style: { fill: secondaryTextColor, fontSize: 12, textAnchor: 'middle' },
                    }
                  : undefined
              }
            />
