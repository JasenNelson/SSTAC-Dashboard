...
describe('matrix options evidence library helpers', () => {
  it('builds the default view from repo-local catalog records', () => {
    ...
    // valueGroups count updated 2026-05-31: +213 Protocol 28 HH-soil TRV records (d0c00003) +34 IRIS records (d0c00011+d0c00013).
    expect(view.valueGroups).toHaveLength(395);
    // approvedSourceBacked updated 2026-05-31: +34 IRIS records (evidence_support_status=approved_source_backed).
    expect(view.audit.values.approvedSourceBacked).toBe(118);
    // pendingSourceLocator updated 2026-05-31: +213 p28 records (evidence_support_status=pending_source_locator).
    ...
    // availableOptions updated 2026-05-31: +213 p28 records +34 IRIS records (default_status=available_option).
    expect(view.audit.values.availableOptions).toBe(337);
