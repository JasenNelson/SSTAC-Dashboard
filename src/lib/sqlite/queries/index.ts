...
export function getSubmissions(orderBy: 'imported_at' | 'evaluation_completed' = 'imported_at'): Submission[] {
  // Validate orderBy to prevent SQL injection even if caller bypassed types
  const safeOrderBy = ['imported_at', 'evaluation_completed'].includes(orderBy) ? orderBy : 'imported_at';
  const sql = `
