...
export function getSubmissions(orderBy: 'imported_at' | 'evaluation_completed' = 'imported_at'): Submission[] {
  // Validate orderBy parameter at runtime to prevent SQL injection
  const validOrderColumns = ['imported_at', 'evaluation_completed'];
  const safeOrderBy = validOrderColumns.includes(orderBy) ? orderBy : 'imported_at';
