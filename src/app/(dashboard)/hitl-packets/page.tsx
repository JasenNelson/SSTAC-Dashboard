  ...
  return (
    <ErrorBoundary>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-slate-900">HITL Packet Review</h1>
        ...
        {sessions.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {sessions.map((session) => (
              <Link
                ...
                className="block border border-slate-200 rounded-lg p-5 hover:shadow-md transition-shadow bg-white"
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <FileText className="h-5 w-5 text-sky-500" />
                    <h3 className="font-mono text-sm font-medium text-slate-900 truncate max-w-[200px]">
                      {session.sessionId}
                    </h3>
                  </div>
          ...
          <div className="text-center py-12 bg-white rounded-lg shadow-sm border border-slate-200">
            ...
            <h3 className="mt-4 text-lg font-medium text-slate-900">No packets found</h3>
            <p className="mt-2 text-sm text-slate-500">
              Run the evaluation engine with <code className="bg-slate-100 px-1 rounded">--packet</code> to
