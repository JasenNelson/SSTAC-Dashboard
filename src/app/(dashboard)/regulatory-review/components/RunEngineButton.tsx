...
export default function RunEngineButton({
  ...
  const getButtonClass = () => {
    if (status === 'success') {
      return 'bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 cursor-default';
    }
    if (status === 'error') {
      return 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/50';
    }
    if (status === 'running') {
      return 'bg-yellow-50 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400 cursor-wait';
    }
    ...
    <>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-lg mx-4" role="dialog" aria-modal="true" aria-labelledby="run-engine-title">
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700">
              <h3 id="run-engine-title" className="text-lg font-semibold text-slate-900 dark:text-white">
                Run Evaluation Engine
              </h3>
              <button
                onClick={() => setShowModal(false)}
                className="p-1 text-slate-400 hover:text-slate-500 dark:hover:text-slate-300 rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 dark:ring-offset-slate-800"
                aria-label="Close"
              >
                <X className="w-5 h-5" />
              </button>
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              <div className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-4">
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500 dark:text-slate-400">Submission ID:</span>
                  <span className="font-medium text-slate-900 dark:text-white">{submissionId}</span>
                </div>
                <div className="flex justify-between text-sm mt-1">
                  <span className="text-slate-500 dark:text-slate-400">Site:</span>
                  <span className="font-medium text-slate-900 dark:text-white">{siteId}</span>
                </div>
              </div>

              {/* Mode Selection */}
              <div className="space-y-3">
                <label className="flex items-center space-x-3 p-3 border border-slate-200 dark:border-slate-600 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/50">
                  <input
                    type="radio"
                    name="mode"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="font-medium text-slate-900 dark:text-white">Import Latest Results</span>
                      <p className="text-sm text-slate-500 dark:text-slate-400">Import from most recent evaluation output file (recommended)</p>
                    </div>
                  </div>
                </label>
                ...
                <label className="flex items-center space-x-3 p-3 border border-slate-200 dark:border-slate-600 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/50">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="font-medium text-slate-900 dark:text-white">Run Engine (Existing Extractions)</span>
                      <p className="text-sm text-slate-500 dark:text-slate-400">Re-run evaluation on extraction files in the system</p>
                    </div>
                  </div>
                </label>
                ...
                <label className="flex items-center space-x-3 p-3 border border-slate-200 dark:border-slate-600 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/50">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="font-medium text-slate-900 dark:text-white">Upload New Documents</span>
                      <p className="text-sm text-slate-500 dark:text-slate-400">Upload extraction JSON files to process</p>
                    </div>
                  </div>
                </label>
              </div>
              ...
              {runMode === 'upload' && (
                <div className="space-y-3">
                  <div className="border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-lg p-6 text-center hover:border-emerald-400 dark:hover:border-emerald-500 transition-colors">
                    <Upload className="w-8 h-8 mx-auto text-slate-400 dark:text-slate-500 mb-2" />
                    <label className="cursor-pointer">
                      <span className="text-sm text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 font-medium">
                        Choose files
                      </span>
                    ...
                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">JSON or PDF files</p>
                  </div>
                  ...
                  {selectedFiles.length > 0 && (
                    <div className="space-y-2">
                      {selectedFiles.map((file, index) => (
                        <div
                          key={index}
                          className="flex items-center justify-between bg-slate-50 dark:bg-slate-700/50 rounded px-3 py-2"
                        >
                ...
                >
                  {result.success ? (
                    <div className="space-y-3">
                      ...
                      {result.importResult && (
                        <div className="text-sm text-green-700 dark:text-green-400 bg-green-100 dark:bg-green-900/30 rounded px-3 py-2">
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 rounded-b-lg">
              <button
                ...
                className="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 dark:ring-offset-slate-800"
              >
