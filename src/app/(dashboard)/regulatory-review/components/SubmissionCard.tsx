...
function StatusBadge({ status }: { status: string }) {
  const statusConfig: Record<string, { bg: string; text: string; label: string }> = {
    ...
    in_progress: { bg: 'bg-sky-100 dark:bg-sky-900/30', text: 'text-sky-800 dark:text-sky-200', label: 'In Progress' },
    complete: { bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-800 dark:text-green-200', label: 'Complete' },
  };
    ...
    <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-6 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{submission.type}</p>
        </div>
      ...
      <div className="space-y-3 mb-4">
        <div className="flex items-center text-sm">
          <svg
            className="h-4 w-4 text-slate-400 dark:text-slate-500 mr-2"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
        ...
        <div className="flex items-center text-sm">
          ...
          <span className="text-slate-500 dark:text-slate-400">{submission.submittedBy}</span>
        </div>
        <div className="flex items-center text-sm">
          <svg
            className="h-4 w-4 text-slate-400 dark:text-slate-500 mr-2"
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-slate-600 dark:text-slate-300">Review Progress</span>
          <span className="text-sm text-slate-500 dark:text-slate-400">{submission.totalItems} items</span>
        </div>
        <ProgressBar
      ...
      <div className="flex flex-col gap-2">
        <Link
          ...
          className="inline-flex items-center justify-center w-full px-4 py-2 text-sm font-medium text-sky-600 dark:text-sky-300 bg-sky-50 dark:bg-sky-900/30 rounded-lg hover:bg-sky-100 dark:hover:bg-sky-900/50 transition-colors"
        >
          View Submission
