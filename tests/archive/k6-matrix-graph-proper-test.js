// K6 Proper Matrix Graph Test
// Tests actual matrix graph data point creation and pairing
// Run with: k6 run k6-matrix-graph-proper-test.js
...
export const options = {
  ...
  thresholds: {
    http_req_duration: ['p(95)<10000'], // 10 second timeout for complex queries
...
export default function() {
  console.log('\n🎯 Starting Comprehensive Matrix Graph Test - CEW Polls Only');
  ...
  // Step 2: Test ALL matrix graph question pairs
  console.log('\n🗳️ Step 2: Testing ALL matrix graph question pairs...');
  ...
  // Step 4: Test multiple CEW sessions (simulate page refresh)
  console.log('\n🔄 Step 4: Testing multiple CEW sessions (page refresh simulation)...');
  
  // Submit another round of votes with different options
  const session2Votes = [];
  for (const pair of holisticPairs) {
    const q1Result = submitVote('/cew-polls/holistic-protection', pair.q1, Math.floor(Math.random() * 5), 'CEW2025');
    const q2Result = submitVote('/cew-polls/holistic-protection', pair.q2, Math.floor(Math.random() * 5), 'CEW2025');
    session2Votes.push({ q1: q1Result.success, q2: q2Result.success });
  }
  ...
  const session2Success = session2Votes.filter(v => v.q1 && v.q2).length;
  console.log(`📊 Session 2: ${session2Success}/${session2Votes.length} pairs submitted successfully`);
  
  // Wait for second session processing
  sleep(3);
  
  // Final data check
  const finalAll = getMatrixGraphData('all');
  const finalCew = getMatrixGraphData('cew');
  
  const totalNewAllDataPoints = finalAll.dataPoints - baselineAll.dataPoints;
  const totalNewCewDataPoints = finalCew.dataPoints - baselineCew.dataPoints;
  
  console.log(`📊 Final - All: ${finalAll.dataPoints} (+${totalNewAllDataPoints}), CEW: ${finalCew.dataPoints} (+${totalNewCewDataPoints})`);
  
  // Step 5: Validate results
  console.log('\n✅ Step 5: Validating results...');
  
  const allChecks = check(null, {
    'Matrix API responds': () => finalAll.success,
    'Votes submitted successfully': () => totalVotesSuccessful > 0,
    'New CEW data points created': () => totalNewCewDataPoints > 0,
    'Total data points increased': () => totalNewAllDataPoints > 0,
    'Multiple sessions working': () => session2Success > 0,
  });
  ...
  if (allChecks) {
    ...
    console.log(`✅ Created ${totalNewCewDataPoints} new CEW data points`);
    console.log(`✅ Total data points increased by ${totalNewAllDataPoints}`);
    console.log(`✅ Multiple CEW sessions working (${session2Success} pairs in session 2)`);
  } else {
    ...
    console.log(`❌ Expected new CEW data points: >0, got: ${totalNewCewDataPoints}`);
    console.log(`❌ Expected total increase: >0, got: ${totalNewAllDataPoints}`);
  }
...
export function handleSummary(data) {
  return {
    'matrix-graph-test-results.json': JSON.stringify({
