...
function Get-Relevant([object[]]$Rows, [hashtable]$ByPid, [int]$ParentPid, [int]$CheckerPid, [string]$Runtime) {
    $result = @()
    foreach ($row in $Rows) {
...
try {
    ...
    $relevant = @(Get-Relevant $rows $byPid $RunParentPid $checkerPid $runtime)
