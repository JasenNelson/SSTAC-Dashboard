...
function Test-NightlyReadableFile([string]$Path) {
...
function Complete-NightlyRun([int]$NativeExitCode, [string]$TerminalState) {
    ...
    try {
        ...
        if ($finalHead -cmatch '^[0-9a-f]{40}$') { $autofollowFinalHead = $finalHead }
        $buildStampOid = $null
        $buildStampError = $null
        try {
            $stampText = Get-Content -LiteralPath (Join-Path $RepoRoot 'wiki\.build-stamp') -Raw -ErrorAction Stop
            $stampMatches = [regex]::Matches($stampText, '(?m)^HEAD:\s*([0-9a-f]{40})\s*$')
            if ($stampMatches.Count -ne 1) { throw 'build stamp must contain exactly one canonical HEAD line' }
            $buildStampOid = $stampMatches[0].Groups[1].Value
        } catch {
            $buildStampError = $_.Exception.Message
            Write-Host "FAIL: build stamp: $buildStampError"
            $finalState = 'FAILED'
            $finalExit = 1
        }
        $serveGateResult = if ($serveGateSummary -match '^allowed=True;' -and
            $finalHead -cmatch '^[0-9a-f]{40}$' -and
            $requiredRefOid -cmatch '^[0-9a-f]{40}$' -and
            $finalHead -ceq $requiredRefOid -and
            $buildStampOid -ceq $requiredRefOid) { 'PASS' } else { 'FAIL' }
...
$requiredInstalledTooling = @(
    ...
    $pythonExe
    $configFile
    (Join-Path $normalizedRepoRoot "tooling\wiki\graphify_guardrail.ps1")
    (Join-Path $normalizedRepoRoot "tooling\wiki\gen_docs_scope.py")
    (Join-Path $normalizedRepoRoot "tooling\wiki\canonicalize_graph.py")
    (Join-Path $normalizedRepoRoot "tooling\wiki\scan_secrets.py")
    (Join-Path $normalizedRepoRoot "tooling\wiki\graph_smoke.py")
    (Join-Path $normalizedRepoRoot "tooling\wiki\publish_wiki.py")
    (Join-Path $normalizedRepoRoot "tooling\wiki\wiki_compile.py")
    (Join-Path $normalizedRepoRoot "tooling\wiki\wiki_lint.py")
)
if (-not ($SkipLabeling -and $SkipSemantic)) {
    $requiredInstalledTooling += (Join-Path $normalizedRepoRoot "tooling\wiki\ollama_lock.ps1")
}
if (-not $SkipSemantic) {
    $requiredInstalledTooling += (Join-Path $normalizedRepoRoot "tooling\wiki\semantic_extract.ps1")
    $requiredInstalledTooling += (Join-Path $normalizedRepoRoot "tooling\wiki\promotion.py")
}
$unreadableInstalledTooling = @(
    $requiredInstalledTooling | Where-Object { -not (Test-NightlyReadableFile $_) }
)
if ($unreadableInstalledTooling.Count -gt 0) {
    $autofollowDecision = "REFUSED_UNEXPECTED"
    $autofollowResult = "FAIL"
    $autofollowRejectionReason = "required installed tooling absent or unreadable: $($unreadableInstalledTooling -join ', ')"
    Write-Host "FAIL: Installed runtime preflight: $autofollowRejectionReason"
    Complete-NightlyRun 1 'FAILED'
}
...
if ($autofollowStartingHeadExit -ne 0 -or
    $autofollowStartingHeadRaw.Count -ne 1 -or
    $autofollowStartingHeadRaw[0].Trim() -cnotmatch '^[0-9a-f]{40}$') {
    $autofollowDecision = "REFUSED_UNEXPECTED"
    $autofollowResult = "FAIL"
    $autofollowRejectionReason = "installed HEAD rev-parse failed or returned an invalid OID"
    Write-Host "FAIL: Installed runtime preflight: $autofollowRejectionReason"
    Complete-NightlyRun 1 'FAILED'
}
...
$autofollowRejectionReason = ""
$n0Head = $autofollowStartingHead
$serveGateRequiredRef = "INSTALLED_RUNTIME"
Write-Host "PASS: Installed runtime decision: $autofollowDecision ($autofollowStartingHead)"

# This count is human-readable receipt text only; publication uses the tracked-clean gate below.
$n0PorcelainLines = @(git -C $RepoRoot status --porcelain).Count

Write-Host "--- N1 SCOPE+HASH ---"
...
if ($semanticSkippedReason) {
    ...
    $ollamaLockPath = Join-Path $RepoRoot "tooling\wiki\ollama_lock.ps1"
    if (-not (Test-NightlyReadableFile $ollamaLockPath)) {
        Write-Host "FAIL: N5 lock tooling is absent or unreadable"
        Complete-NightlyRun 1 'FAILED'
    }
    try { . $ollamaLockPath }
    catch {
        Write-Host "FAIL: N5 lock tooling load failed: $($_.Exception.Message)"
        Complete-NightlyRun 1 'FAILED'
    }
    if ($n5Plan.RunSemantic) {
        $semanticExtractPath = Join-Path $RepoRoot "tooling\wiki\semantic_extract.ps1"
        $promotionPath = Join-Path $RepoRoot "tooling\wiki\promotion.py"
        $missingSemanticTooling = @(
            $semanticExtractPath
            $promotionPath
        ) | Where-Object { -not (Test-NightlyReadableFile $_) }
        if ($missingSemanticTooling.Count -gt 0) {
            Write-Host "FAIL: semantic tooling is absent or unreadable: $($missingSemanticTooling -join ', ')"
            Complete-NightlyRun 1 'FAILED'
        }
    }
    ...
    if ($null -eq $h) {
        ...
        try {
            ...
            if ($n5Plan.RunSemantic -and -not $gpuOrphanRisk -and -not $step5Fail) {
                ...
                $semArgs = @('-NoProfile','-ExecutionPolicy','Bypass','-File', $semanticExtractPath, '-SkipLock', '-TimeoutSec', $cfgTimeoutSemInner, '-EvidencePath', $semanticEvidencePath, '-EvidenceRunId', $runId)
                $n5MutationAttempted = $true
                ...
                if ($step5Fail) {
                    ...
                    if ($currPorcelain.Count -ne 0) {
                        ...
                        try {
                            ...
                            $promArgs = @($promotionPath, '--graph', (Join-Path $RepoRoot "graphify-out\graph.json"), '--state', $promotionCandidate, '--commit', (git -C $RepoRoot rev-parse --short HEAD).Trim(), '--report')
                            & $pythonExe @promArgs
                            if ($LASTEXITCODE -ne 0) {
...
if ($graphOrphanRisk -or -not $n1BuildOk -or $step2Status -ne "OK" -or $step5Status -eq "FAIL" -or -not $prePublishGraphIntegrityOk) {
    ...
    $trackedStatus = @(git -c gc.auto=0 -c maintenance.auto=false -C $RepoRoot status --porcelain --untracked-files=no)
    $trackedStatusExit = $LASTEXITCODE
    $trackedClean = ($trackedStatusExit -eq 0 -and $trackedStatus.Count -eq 0)
    $currentHeadRaw = @(git -c gc.auto=0 -c maintenance.auto=false -C $RepoRoot rev-parse HEAD)
    $currentHeadExit = $LASTEXITCODE
    $currentHead = if ($currentHeadExit -eq 0 -and $currentHeadRaw.Count -eq 1) { $currentHeadRaw[0].Trim() } else { "" }
    $headUnchanged = ($currentHead -cmatch '^[0-9a-f]{40}$' -and $currentHead -ceq $autofollowStartingHead)
    if ($currentHead -cmatch '^[0-9a-f]{40}$') { $autofollowFinalHead = $currentHead }

    $serveGateOk = ($trackedClean -and $headUnchanged)
    $serveGateReasons = @()
    if ($trackedStatusExit -ne 0) { $serveGateReasons += "Tracked status failed" }
    elseif (-not $trackedClean) { $serveGateReasons += "Tracked files dirty" }
    if ($currentHead -cnotmatch '^[0-9a-f]{40}$') { $serveGateReasons += "Installed HEAD unreadable" }
    elseif (-not $headUnchanged) { $serveGateReasons += "HEAD changed" }
    $serveGateSummary = "allowed=$serveGateOk; required_ref=$serveGateRequiredRef; required_ref_oid=$autofollowStartingHead; fetched_oid=; head=$currentHead"
...
$commitsBehind = 0
$ageDays = 0
$freshnessStr = "installed runtime: $autofollowStartingHead"
