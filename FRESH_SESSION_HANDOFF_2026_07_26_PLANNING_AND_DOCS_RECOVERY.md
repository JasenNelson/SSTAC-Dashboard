...
Draft. Implements the hardened publication foundation: an audited publication
...
### This branch - `feat/option-c-candidate-lifecycle-2026-07-25`
...
**This branch is explicitly NON-MERGEABLE before #752 lands**, and its existence
implies **no Supabase deployability**. Its SQL is a design/publication primitive,
not an applied append-only migration.
...
| # | Decision | State |
|---|---|---|
| D1(a)(b) | Member publication semantics: `coordinate_quality_tier` exposure, and `visible_sample_suppression_key` | **OPEN.** Reviewers settled the suppression key at P3 (architectural hardening, not a privilege leak): it is non-NULL only under the same predicate that gates the member sample RPC, so a recipient can already derive it. |
