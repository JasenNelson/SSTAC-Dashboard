...
| 5 | Decoupled semantic-auth watchdog design (per-night eval reads only the artifact; the aggregate starvation watchdog reads a durable tracked `wiki_nightly_config.json` `semantic_mode`) | **[Phase-4-6-only]** | No nightly scripts exist in SSTAC yet (confirmed absent in the phase-state audit and by the `tooling/wiki/` glob). This is Phase 5 design detail with nothing yet built to check it against. [HISTORICAL as of 2026-07-22; SUPERSEDED 2026-08-06 -- `tooling/wiki/` nightly scripts now exist and `SSTAC-Wiki-Nightly` is registered and running. See `docs/WIKI_KB_OPERATIONS_2026_07.md` sections 1 and 12.] |
