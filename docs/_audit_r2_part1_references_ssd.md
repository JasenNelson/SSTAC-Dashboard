# UI/UX Audit Round 2 -- Part 1: References & Values, SSD Workbench

Source tree read: `C:\Projects\SSTAC-Dashboard-worktrees\triage-20260816` at `bfbab1c9`.
Findings-only. No source file was modified, no gate/build/dev-server was run.

Palette basis: `src/app/globals.css` defines `@theme inline` over semantic tokens only
(lines 37-58) and never redefines `--color-sky-*` / `--color-amber-*` / `--color-emerald-*`
/ `--color-slate-*`, so the Tailwind default palette applies. All ratios below were computed
from the default hexes using the WCAG relative-luminance formula.
