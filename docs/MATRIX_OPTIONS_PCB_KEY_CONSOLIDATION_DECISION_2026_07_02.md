...
### 2. The PCB-family keys in `src/lib/matrix-options/substanceLibrary.ts`
...
| `polychlorinated_biphenyls_pcbs` | generic | Polychlorinated biphenyls (PCBs) | (generic IRIS PCBs key) | - | 2.0 | - | - | - | - | the generic IRIS PCBs source that `total_pcbs_aroclor_1254` BORROWS its sf_oral 2.0 from; flagged in triage as a potential 3rd redundant PCB-family key |
| `biphenyl` | ~873 | Biphenyl | 4.01 | - | - | - | - | - | - | the un-chlorinated PARENT compound, NOT a PCB -- explicitly OUT OF SCOPE (do not fold in) |

Note: `pcbs_non_coplanar` (HC RfD 0.00001) also exists and overlaps `total_pcbs_aroclor_1254`'s 2.0e-5 IRIS RfD; it is a separately-deferred HITL item that a congener policy must also address.
