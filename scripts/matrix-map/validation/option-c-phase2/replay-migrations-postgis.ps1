...
param(
  ...
  # from. Defaults to the PR #752 source worktree so pre-existing behaviour is
  # preserved exactly; the candidate-lifecycle branch passes its own worktree so
  # the replay can run against the MODIFIED SQL.
  [Parameter(Mandatory = $false)]
  [string]$RepoRoot = 'C:\tmp\sstac-option-c-phase2-2026-07-24'
)
