// Owner-run promotion helper for the BC WLRS subsistence fish-ingestion-rate record. Plain ASCII only.
//
// WHY THIS EXISTS
// The Phase C "C-BC" BC-frame now offers a SECOND receptor scenario (subsistence fisher) alongside
// the already-promoted recreational fisher. The BC WLRS 2023 subsistence fisher rate (0.22 kg/day,
// 220 g/day) -- corroborated by the TWN Burrard Inlet WQO 2021 -- is the correct subsistence-
// receptor candidate for the human-health food pathway. This tool performs the exact, coupled
// promotion of EXACTLY the subsistence-fisher VALUE record and its WLRS SOURCE record, fails closed
// on any precondition, and is idempotent.
//
// This helper was forked from promote-wlrs-default.mjs (the recreational-fisher precedent) and
...
// subsistence record's applicability string is repaired only when the OWNER runs this with --apply
...
//    WLRS subsistence rate (0.22 kg/day = 220 g/day) against the primary BC WLRS 2023 PDF
//    ("Derivation of Screening Values for Contaminants in Fish Tissue", ISBN 978-1-0399-0019-6,
//    Table 2, adult subsistence) and file that PDF in the reference library + Zotero. Running with
//    --apply attests to that verification.
//  - SCOPE (rule 2): only the subsistence-fisher record is promoted as the BC default candidate.
//    The recreational (already approved) and low-level WLRS rows remain as-is (recreational is
//    approved; low-level stays needs_review as a further alternative).
...
//   node scripts/matrix-options/promote-wlrs-subsistence.mjs --reviewer "J. Nelson" --date 2026-06-13
//   node scripts/matrix-options/promote-wlrs-subsistence.mjs --reviewer "J. Nelson" --date 2026-06-13 --apply
//
...
export const WLRS_SUBSISTENCE_PROMOTION_VALUE_ID = 'pv-wlrs-2023-ir-food-subsistence-bc';
export const WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID = 'src-bc-wlrs-fish-tissue-screening-2023';
...
export function planPromotion(paramValues, sources, _opts) {
  ...
  const valueRecord = paramValues.find(
    (r) => r.parameter_value_id === WLRS_SUBSISTENCE_PROMOTION_VALUE_ID,
  );
  if (!valueRecord) {
    throw new Error(
  ...
  if (!Array.isArray(valueRecord.evidence_items) || valueRecord.evidence_items.length === 0) {
    throw new Error(
      'Precondition failed: ' + WLRS_SUBSISTENCE_PROMOTION_VALUE_ID + ' has no evidence_items',
    );
  }
  // Fail-closed IDENTITY check: only ever promote the EXACT intended WLRS subsistence
  ...
  if (identityMismatch.length > 0) {
    throw new Error(
      'Precondition failed: ' + WLRS_SUBSISTENCE_PROMOTION_VALUE_ID +
  ...
  if (!Array.isArray(valueRecord.source_ids) ||
      ...
      valueRecord.source_ids[0] !== WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID) {
    throw new Error(
      'Precondition failed: ' + WLRS_SUBSISTENCE_PROMOTION_VALUE_ID + ' source_ids must be EXACTLY ["' +
      WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID + '"] (actual: ' + JSON.stringify(valueRecord.source_ids) +
  ...
  const staleNestedSourceRefs = nestedSourceRefs.filter(
    (sid) => sid !== WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID,
  );
  if (staleNestedSourceRefs.length > 0) {
    throw new Error(
      'Precondition failed: ' + WLRS_SUBSISTENCE_PROMOTION_VALUE_ID + ' has nested provenance source ' +
      ...
      WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID + '"; a stale nested source reference ' +
  ...
  const sourceRecord = sources.find(
    (s) => s.source_id === WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID,
  );
  if (!sourceRecord) {
    throw new Error(
      'Precondition failed: source record not found in sources.json: ' +
      WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID,
    );
  }
  ...
  if (!sourceAlreadyDone && !sourceExpectedPre) {
    throw new Error(
      'Precondition failed: source ' + WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID +
  ...
  if (allRoles.includes('policy_compilation') || allRoles.includes('reference_mining')) {
    throw new Error(
      'Precondition failed: ' + WLRS_SUBSISTENCE_PROMOTION_VALUE_ID + ' has a source role of ' +
...
const BANNER = [
  '=============================================================================',
  ' promote-wlrs-subsistence.mjs -- owner-run WLRS subsistence IR-food promotion',
  '=============================================================================',
  '',
  'OWNER VERIFICATION REQUIRED before --apply:',
  '  Verify the WLRS subsistence rate (0.22 kg/day = 220 g/day) against the',
  '  primary BC WLRS 2023 PDF (ISBN 978-1-0399-0019-6, Table 2, adult subsistence',
  ...
  '',
  'SCOPE: only the subsistence-fisher value + its WLRS source are promoted.',
  '  The recreational (already approved) and low-level WLRS rows are not touched.',
...
const HELP = [
  'promote-wlrs-subsistence.mjs -- owner-run BC WLRS subsistence IR-food promotion tool.',
  '',
  'Usage:',
  '  node scripts/matrix-options/promote-wlrs-subsistence.mjs --reviewer "<id>" --date YYYY-MM-DD [--apply]',
  ...
  '',
  'Targets:',
  '  VALUE : pv-wlrs-2023-ir-food-subsistence-bc   (parameter_values.json)',
...
function printPlan(plan, opts) {
  if (plan.promoteValue) {
    const r = plan.valueRecord;
    console.log('  VALUE   PROMOTE  ' + WLRS_SUBSISTENCE_PROMOTION_VALUE_ID);
    ...
    console.log('  VALUE   SKIP     ' + WLRS_SUBSISTENCE_PROMOTION_VALUE_ID + ': already in target state (no-op)');
  }

  if (plan.promoteSource) {
    const s = plan.sourceRecord;
    console.log('  SOURCE  PROMOTE  ' + WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID);
    ...
    console.log('  SOURCE  SKIP     ' + WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID +
...
function main() {
  ...
  if (valueWillBeApproved && sourceWillBeVerified && !effUrl && !effKey) {
    throw new Error(
      'Provenance guard: ' + WLRS_SUBSISTENCE_PROMOTION_VALUE_ID + ' would be approved against source ' +
      WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID + ' (direct_source_verified), but that source has no durable' +
