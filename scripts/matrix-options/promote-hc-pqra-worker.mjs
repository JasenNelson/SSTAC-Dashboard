// Owner-run promotion helper for the HC PQRA v4.0 commercial/industrial WORKER direct-contact
// receptor profile. Plain ASCII only.
//
// WHY THIS EXISTS
// The human-health direct-contact pathway frame for the worker receptor requires 5 worker-specific
// parameter value records to be promoted from needs_review to approved before they can seed a
// frame default profile. Each record is backed by the same source (Health Canada PQRA v4.0, 2024).
// This tool promotes EXACTLY those 5 records + the source, fails closed on any precondition, and
// is idempotent. (The worker scenario also reuses BW-adult 70.7 kg + AT-cancer-lifetime 80 yr,
// already approved with the residential-adult scenario in #308 -- those are NOT touched here.)
//
// It mirrors promote-hc-pqra-adult.mjs in structure, style, and fail-closed discipline, adapted
// for the 5-record worker batch.
//
...
//    5 values against the primary Health Canada PQRA v4.0 PDF (Cat. H129-114/2023E-PDF, ISBN
//    978-0-660-68497-0, March 2024). Running with --apply attests to that verification.
//  - SCOPE (rule 2): only the 5 listed records are promoted. default_status is NEVER modified
...
//    canonical_source_status = 'direct_source_verified' (promoted by the toddler/adult runs, #305/#308).
//    On the real catalog the SOURCE will normally SKIP (already-done). The source promotion LOGIC
//    is kept identical (it handles the already-done case gracefully).
...
//   node scripts/matrix-options/promote-hc-pqra-worker.mjs --reviewer "J. Nelson" --date 2026-06-13
//   node scripts/matrix-options/promote-hc-pqra-worker.mjs --reviewer "J. Nelson" --date 2026-06-13 --apply
//
...
// The 5 not-yet-approved WORKER (commercial/industrial) receptor records. BW-adult (70.7) and
// AT-cancer-lifetime (80) are SHARED with the residential-adult scenario and were already approved
// (#308); they are intentionally NOT in this list (reused, not re-promoted).
export const HC_PQRA_WORKER_PROMOTION_VALUE_IDS = [
  'pv-hc-pqra-v4-2024-ef-commercial-ca',
  'pv-hc-pqra-v4-2024-ed-commercial-ca',
  'pv-hc-pqra-v4-2024-ir-sed-worker-ca',
  'pv-hc-pqra-v4-2024-sa-total-worker-ca',
  'pv-hc-pqra-v4-2024-af-sed-other-worker-ca',
];
export const HC_PQRA_WORKER_PROMOTION_SOURCE_ID = 'src-health-canada-pqra-v4-2024';

// Expected frame-eligible identity per record (fields classifyCandidate / getFrameSeedCandidateEligibility
// examine). Any mismatch -> fail closed. Ordered to match HC_PQRA_WORKER_PROMOTION_VALUE_IDS above.
const EXPECTED_IDENTITIES = [
  // pv-hc-pqra-v4-2024-ef-commercial-ca (Table 2: 5 d/wk x 48 wk/yr = 240 days/yr)
  {
  ...
  // pv-hc-pqra-v4-2024-ed-commercial-ca (Table 2: 35 yr commercial/industrial)
  {
    jurisdiction: 'general',
  ...
  // pv-hc-pqra-v4-2024-ir-sed-worker-ca (Appendix E worker incidental ingestion; MassDEP 2002)
  {
    jurisdiction: 'general',
  ...
  // pv-hc-pqra-v4-2024-sa-total-worker-ca (Appendix E total body; owner-attested correction 1640 -> 16640)
  {
    jurisdiction: 'general',
  ...
  // pv-hc-pqra-v4-2024-af-sed-other-worker-ca (Appendix E surfaces-other-than-hands; Kissel)
  {
    jurisdiction: 'general',
...
function planOneValueRecord(paramValues, valueId, expectedIdentity) {
  ...
  if (!Array.isArray(valueRecord.source_ids) ||
      ...
      valueRecord.source_ids[0] !== HC_PQRA_WORKER_PROMOTION_SOURCE_ID) {
    throw new Error(
      'Precondition failed: ' + valueId + ' source_ids must be EXACTLY ["' +
      HC_PQRA_WORKER_PROMOTION_SOURCE_ID + '"] (actual: ' + JSON.stringify(valueRecord.source_ids) +
  ...
  const staleNestedSourceRefs = nestedSourceRefs.filter(
    (sid) => sid !== HC_PQRA_WORKER_PROMOTION_SOURCE_ID,
  );
  if (staleNestedSourceRefs.length > 0) {
    throw new Error(
      ...
      HC_PQRA_WORKER_PROMOTION_SOURCE_ID + '"; a stale nested source reference ' +
...
// Computes the promotion plan for all 5 value records + the source. Enforces ALL preconditions
...
export function planPromotion(paramValues, sources, _opts) {
  ...
  if (!sourceAlreadyDone && !sourceExpectedPre) {
    throw new Error(
      'Precondition failed: source ' + HC_PQRA_WORKER_PROMOTION_SOURCE_ID +
...
const BANNER = [
  '=========================================================================',
  ' promote-hc-pqra-worker.mjs -- owner-run HC PQRA v4.0 worker-receptor promotion',
  '=========================================================================',
  '',
  'OWNER VERIFICATION REQUIRED before --apply:',
  '  Verify the 5 worker-receptor values against the primary Health Canada PQRA v4.0 PDF',
  '    (Cat. H129-114/2023E-PDF, ISBN 978-0-660-68497-0, March 2024).',
  '  NOTE the SA value: Appendix E prints "1 640" cm2 for the worker total body; the catalog',
  '  carries the owner-attested correction 16640 cm2 (apparent HC typesetting error; impossible',
  '  as printed). Running --apply attests to that correction + all 5 values.',
  '',
  'SOURCE NOTE:',
  '  src-health-canada-pqra-v4-2024 is expected to already be direct_source_verified',
  '  (promoted by the toddler/adult runs, #305/#308). On the real catalog the SOURCE will SKIP.',
  ...
  '',
  'SCOPE: 5 worker-receptor value records + 1 source are promoted. default_status',
  '  is NOT changed (stays available_option). The FRAME_DEFAULT_PROFILES row is the owner',
  '  activation step, not this tool. (BW-adult 70.7 + AT-cancer 80 are already approved, #308.)',
  '',
].join('\n');

const HELP = [
  'promote-hc-pqra-worker.mjs -- owner-run HC PQRA v4.0 commercial/industrial worker profile promotion.',
  '',
  'Usage:',
  '  node scripts/matrix-options/promote-hc-pqra-worker.mjs --reviewer "<id>" --date YYYY-MM-DD [--apply]',
  ...
  '',
  'Targets (5 worker-receptor VALUE records + 1 SOURCE):',
  '  VALUE : pv-hc-pqra-v4-2024-ef-commercial-ca',
  '  VALUE : pv-hc-pqra-v4-2024-ed-commercial-ca',
  '  VALUE : pv-hc-pqra-v4-2024-ir-sed-worker-ca',
  '  VALUE : pv-hc-pqra-v4-2024-sa-total-worker-ca',
  '  VALUE : pv-hc-pqra-v4-2024-af-sed-other-worker-ca',
...
function printPlan(plan, opts) {
  for (let i = 0; i < plan.valueResults.length; i++) {
    ...
    const valueId = HC_PQRA_WORKER_PROMOTION_VALUE_IDS[i];
    if (vr.promoteValue) {
      const r = vr.valueRecord;
  ...
  if (plan.promoteSource) {
    ...
    console.log('  SOURCE  PROMOTE  ' + HC_PQRA_WORKER_PROMOTION_SOURCE_ID);
    console.log('    canonical_source_status : ' + currentStatus +
      ' -> direct_source_verified');
  } else {
    console.log('  SOURCE  SKIP     ' + HC_PQRA_WORKER_PROMOTION_SOURCE_ID +
...
function main() {
  ...
  if (anyValueWillBeApproved && sourceWillBeVerified && !effUrl && !effKey) {
    throw new Error(
      ...
      HC_PQRA_WORKER_PROMOTION_SOURCE_ID + ' (direct_source_verified), but that source has no durable' +
