...
export function planPromotion(paramValues, sources, _opts) {
  ...
  const staleNestedSourceRefs = nestedSourceRefs.filter(
    (sid) => sid !== BW_PROMOTION_SOURCE_ID,
  );
  if (staleNestedSourceRefs.length > 0) {
    throw new Error(
      'Precondition failed: ' + BW_PROMOTION_VALUE_ID + ' has nested provenance source ' +
      ...
      BW_PROMOTION_SOURCE_ID + '"; a stale nested source reference ' +
