// Guards for promote-iris-pfda-dupe-cg-canonical.mjs (promotion of the PFDA chronic 2e-9
// oral-RfD rows). Synthetic fixtures, independent of catalog state. Plain ASCII.
...
import {
  ...
  planPromotion,
  applyPromotion,
} from '../promote-iris-pfda-dupe-cg-canonical.mjs';

const JURISDICTION = 'US_federal';
const APPLY_OPTS = { reviewer: 'J. Nelson', date: '2026-06-23', apply: true };
...
function makeFixture() {
  ...
  // Add a non-primary sibling in one group (a value we are NOT promoting) -- must stay needs_review.
  records.push({ ...makeValueRecord(PROMOTION_ROWS[0]), parameter_value_id: 'pv-iris-pfas-sibling-not-promoted', value: 999 });
  return { records, sources: makeSources() };
}

describe('promote-iris-pfda-dupe-cg-canonical: scope', () => {
  it('targets exactly the 2 PFDA chronic ids', () => {
    expect(IRIS_PFDA_DUPE_CG_CANONICAL_VALUE_IDS).toHaveLength(2);
    expect(new Set(IRIS_PFDA_DUPE_CG_CANONICAL_VALUE_IDS).size).toBe(2);
    for (const id of IRIS_PFDA_DUPE_CG_CANONICAL_VALUE_IDS) expect(id.startsWith('pv-iris-')).toBe(true);
  });
  it('only PFDA in scope', () => {
    for (const r of PROMOTION_ROWS) {
      expect(r.substanceKey).toBe('perfluorodecanoic_acid_pfda');
    }
    expect(PROMOTION_ROWS.every((r) => r.value === 2e-9)).toBe(true);
  });
  it('every data row carries an EPA IRIS url + a verified value + the RfD-only source', () => {
    for (const r of PROMOTION_ROWS) {
      expect(r.irisUrl).toMatch(/epa\.gov/);
      expect(typeof r.value).toBe('number');
      expect(r.inputKey).toBe('rfd_oral_mg_per_kg_bw_day');
      expect(r.sourceId).toBe('src-us-epa-iris-rfd-table-live');
    }
  });
});

describe('promote-iris-pfda-dupe-cg-canonical: parseArgs + validateApplyOptions', () => {
  it('parses flags', () => {
    expect(parseArgs(['n', 's', '--reviewer', 'J. Nelson', '--date', '2026-06-23', '--apply'])).toMatchObject({ reviewer: 'J. Nelson', date: '2026-06-23', apply: true });
  });
  it('requires reviewer + YYYY-MM-DD date for --apply', () => {
    expect(() => validateApplyOptions({ reviewer: '', date: '2026-06-23' })).toThrow(/reviewer/);
    expect(() => validateApplyOptions({ reviewer: 'x', date: '2026/06/23' })).toThrow(/date/);
    expect(() => validateApplyOptions({ reviewer: 'x', date: '2026-06-23' })).not.toThrow();
  });
});

describe('promote-iris-pfda-dupe-cg-canonical: planPromotion (fail-closed)', () => {
  it('plans all 2 clean rows', () => {
    const { records, sources } = makeFixture();
    const plan = planPromotion(records, sources, APPLY_OPTS);
    expect(plan.valueResults.filter((vr) => vr.promoteValue)).toHaveLength(2);
    expect(plan.sourceResults).toHaveLength(1); // RfD source only (value-only lane), read-only
  });
  it('fails closed if the source is not already direct_source_verified (value-only lane)', () => {
    const { records, sources } = makeFixture();
  ...
  it('throws on a value drifted from the EPA-verified canonical value', () => {
    const { records, sources } = makeFixture();
    records.find((r) => r.parameter_value_id === PROMOTION_ROWS[0].id).value = 7;
  ...
  it('fails closed when another approved member already exists in a group', () => {
    const { records, sources } = makeFixture();
    records.find((r) => r.parameter_value_id === 'pv-iris-pfas-sibling-not-promoted').qa_status = 'approved';
    expect(() => planPromotion(records, sources, APPLY_OPTS)).toThrow(/already has a different approved member/);
  });
});

describe('promote-iris-pfda-dupe-cg-canonical: applyPromotion', () => {
  it('flips qa + canonical + stamps evidence note + value provenance', () => {
    const { records, sources } = makeFixture();
    applyPromotion(records, sources, APPLY_OPTS);
    for (const id of IRIS_PFDA_DUPE_CG_CANONICAL_VALUE_IDS) {
    ...
    // The non-primary sibling is untouched.
    expect(records.find((x) => x.parameter_value_id === 'pv-iris-pfas-sibling-not-promoted').qa_status).toBe('needs_review');
  });
  it('is idempotent (second apply changes nothing; single stamp)', () => {
    ...
    const r0 = records.find((x) => x.parameter_value_id === IRIS_PFDA_DUPE_CG_CANONICAL_VALUE_IDS[0]);
