// Guards for the owner-run BC WLRS subsistence fish-ingestion-rate promotion tool
// (promote-wlrs-subsistence.mjs). Plain ASCII only.
//
...
//
// Mirrors promote-wlrs-default.test.mjs (the recreational-fisher precedent) re-scoped to the
// subsistence records.
...
function makeValueRecord(overrides = {}) {
  return {
    ...
    jurisdiction: 'BC',
    source_ids: [WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID],
    uncertainty: 'Secondary citation pending direct-source verification.',
    review_notes: 'BC WLRS 2023 subsistence fisher fish-ingestion rate (220 g/day).',
    evidence_items: [
      {
        source_id: WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID,
        locator: 'BC WLRS 2023 Table 2 (Fish Ingestion Rates, adults)',
        value_text: 'subsistence fisher (adult Indigenous/subsistence receptor): 220 g/day = 0.22 kg/day',
        extraction_method: 'manual_source_extraction',
        extracted_by: 'claude-sonnet-4-6',
        extracted_at: '2026-06-13',
        qa_status: 'needs_review',
        note: 'n',
        evidence_id: 'ev-' + WLRS_SUBSISTENCE_PROMOTION_VALUE_ID + '-1',
...
describe('promote-wlrs-subsistence: scope constants', () => {
  it('targets exactly the WLRS subsistence value + WLRS 2023 source', () => {
    expect(WLRS_SUBSISTENCE_PROMOTION_VALUE_ID).toBe('pv-wlrs-2023-ir-food-subsistence-bc');
    expect(WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID).toBe('src-bc-wlrs-fish-tissue-screening-2023');
  });
});

describe('promote-wlrs-subsistence: parseArgs', () => {
  it('parses flags and values', () => {
    ...
    expect(a).toMatchObject({
      reviewer: 'J. Nelson', date: '2026-06-13', sourceUrl: 'https://gov.bc.ca/x',
...
describe('promote-wlrs-subsistence: validateApplyOptions', () => {
...
describe('promote-wlrs-subsistence: planPromotion preconditions (fail-closed)', () => {
  ...
  it('throws when the value record is missing', () => {
    const { records, sources } = makeFixture();
    const filtered = records.filter((r) => r.parameter_value_id !== WLRS_SUBSISTENCE_PROMOTION_VALUE_ID);
  ...
  it('throws when source_ids has a SECOND linked source (under-block risk)', () => {
    const { records, sources } = makeFixture({ source_ids: [WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID, 'src-extra'] });
    expect(() => planPromotion(records, sources, APPLY_OPTS)).toThrow(/source_ids must be EXACTLY/);
  });
  ...
  it('throws when a source_relationships role is reference_mining', () => {
    const { records, sources } = makeFixture({
      source_relationships: [{ source_id: WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID, role: 'reference_mining' }],
    });
    expect(() => planPromotion(records, sources, APPLY_OPTS)).toThrow(/policy_compilation\/reference_mining/);
  });
...
describe('promote-wlrs-subsistence: idempotency', () => {
  ...
  it('on a locator-only re-run, returns sourceTouched=true and updates the url', () => {
    ...
    const s = sources.find((x) => x.source_id === WLRS_SUBSISTENCE_PROMOTION_SOURCE_ID);
    expect(s.url).toBe('https://gov.bc.ca/real.pdf');
  });
});

describe('promote-wlrs-subsistence: field edits', () => {
  it('flips qa + evidence + adds attestation + sets direct_source_verified', () => {
    const { records, sources } = makeFixture();
    applyPromotion(records, sources, APPLY_OPTS);
    const r = records.find((x) => x.parameter_value_id === WLRS_SUBSISTENCE_PROMOTION_VALUE_ID);
    ...
    expect(r.value).toBe(0.22); // value untouched
  ...
  it('stamps the applicability field on fresh promotion (Evidence Library renders it verbatim)', () => {
    const { records, sources } = makeFixture({
      applicability: 'BC WLRS 2023 subsistence fisher adult rate. needs_review candidate; not a calculator default.',
    });
    applyPromotion(records, sources, APPLY_OPTS);
    const r = records.find((x) => x.parameter_value_id === WLRS_SUBSISTENCE_PROMOTION_VALUE_ID);
    ...
    const { records, sources } = makeFixture(
      {
        ...DONE_VALUE,
        applicability: 'BC WLRS 2023 subsistence fisher adult rate. needs_review candidate; not a calculator default.',
        uncertainty: 'secondary citation [PROMOTED to approved on 2026-06-13 by J. Nelson]',
        review_notes: 'note [PROMOTED to approved on 2026-06-13 by J. Nelson]',
      },
      { canonical_source_status: 'direct_source_verified' },
    );
