// Guards for the owner-run ACFN WQCIU community-specific food-ingestion-rate promotion tool
// (promote-acfn-foodweb.mjs). Plain ASCII only.
//
...
//
// Mirrors promote-wlrs-subsistence.test.mjs (the WLRS subsistence precedent) re-scoped to the
// ACFN WQCIU community-specific records.
...
function makeValueRecord(overrides = {}) {
  return {
    ...
    source_ids: [ACFN_FOODWEB_PROMOTION_SOURCE_ID],
    uncertainty: 'Secondary citation pending direct-source verification.',
    review_notes: 'ACFN WQCIU 2023 community-specific food ingestion rate (388 g/day).',
    evidence_items: [
      {
        source_id: ACFN_FOODWEB_PROMOTION_SOURCE_ID,
        locator: 'ACFN WQCIU 2023 (community-specific food ingestion rate)',
        value_text: 'ACFN community-specific food ingestion rate: 388 g/day = 0.388 kg/day',
        extraction_method: 'manual_source_extraction',
        extracted_by: 'claude-sonnet-4-6',
        extracted_at: '2026-06-14',
        qa_status: 'needs_review',
        evidence_support_status: 'pending_source_locator',
        evidence_id: 'ev-' + ACFN_FOODWEB_PROMOTION_VALUE_ID + '-1',
...
function makeSourceRecord(overrides = {}) {
  return {
    source_id: ACFN_FOODWEB_PROMOTION_SOURCE_ID,
    short_citation: 'ACFN WQCIU 2023',
    url: 'https://acfn.com/wp-content/uploads/2023/10/wqciu_report.pdf',
    zotero_item_key: 'BHHDGZI9',
    zotero_status: 'pending_owner_export',
    notes: 'ACFN WQCIU 2023; needs_review pending direct-source verification.',
    canonical_source_status: 'needs_direct_source_check',
...
describe('promote-acfn-foodweb: scope constants', () => {
  it('targets exactly the ACFN food-web value + ACFN WQCIU 2023 source', () => {
    expect(ACFN_FOODWEB_PROMOTION_VALUE_ID).toBe('pv-acfn-wqciu-2023-ir-food-community-specific');
    expect(ACFN_FOODWEB_PROMOTION_SOURCE_ID).toBe('src-acfn-wqciu-2023');
  });
});

describe('promote-acfn-foodweb: parseArgs', () => {
  it('parses flags and values', () => {
    const a = parseArgs(['node', 's', '--reviewer', 'J. Nelson', '--date', '2026-06-14',
      '--source-url', 'https://acfn.com/wqciu_report.pdf', '--zotero-key', 'BHHDGZI9', '--apply']);
    expect(a).toMatchObject({
      reviewer: 'J. Nelson', date: '2026-06-14', sourceUrl: 'https://acfn.com/wqciu_report.pdf',
      zoteroKey: 'BHHDGZI9', apply: true,
    });
  });
...
describe('promote-acfn-foodweb: validateApplyOptions', () => {
...
describe('promote-acfn-foodweb: planPromotion preconditions (fail-closed)', () => {
  ...
  it('throws when the value record is missing', () => {
    const { records, sources } = makeFixture();
    const filtered = records.filter((r) => r.parameter_value_id !== ACFN_FOODWEB_PROMOTION_VALUE_ID);
  ...
  it('throws when source_ids does not link the ACFN source', () => {
  ...
  it('throws when source_ids has a SECOND linked source (under-block risk)', () => {
    const { records, sources } = makeFixture({ source_ids: [ACFN_FOODWEB_PROMOTION_SOURCE_ID, 'src-extra'] });
    expect(() => planPromotion(records, sources, APPLY_OPTS)).toThrow(/source_ids must be EXACTLY/);
  });
  it('throws when an evidence_items source_id is a stale (non-ACFN) nested source', () => {
    const { records, sources } = makeFixture({
  ...
  it('throws when a source_relationships source_id is a stale (non-ACFN) nested source', () => {
    const { records, sources } = makeFixture({
  ...
  it('throws when a source_relationships role is reference_mining', () => {
    const { records, sources } = makeFixture({
      source_relationships: [{ source_id: ACFN_FOODWEB_PROMOTION_SOURCE_ID, role: 'reference_mining' }],
    });
    expect(() => planPromotion(records, sources, APPLY_OPTS)).toThrow(/policy_compilation\/reference_mining/);
  });
...
describe('promote-acfn-foodweb: idempotency', () => {
  ...
  // report sourceTouched=true and mutate the url, so the caller's write guard fires.
  it('on a locator-only re-run, returns sourceTouched=true and updates the url', () => {
    ...
    const applied = applyPromotion(records, sources, { ...APPLY_OPTS, sourceUrl: 'https://acfn.com/wqciu_real.pdf' });
    expect(applied.promoteValue).toBe(false);
    expect(applied.promoteSource).toBe(false);
    expect(applied.sourceTouched).toBe(true);
    const s = sources.find((x) => x.source_id === ACFN_FOODWEB_PROMOTION_SOURCE_ID);
    expect(s.url).toBe('https://acfn.com/wqciu_real.pdf');
  });
});

describe('promote-acfn-foodweb: field edits', () => {
  it('flips qa + evidence + adds attestation + sets direct_source_verified', () => {
    const { records, sources } = makeFixture();
    applyPromotion(records, sources, APPLY_OPTS);
    const r = records.find((x) => x.parameter_value_id === ACFN_FOODWEB_PROMOTION_VALUE_ID);
    ...
    expect(r.value).toBe(0.388); // value untouched
  ...
  it('sets the source url + zotero key + linked status from options', () => {
    ...
    applyPromotion(records, sources, { ...APPLY_OPTS, sourceUrl: 'https://acfn.com/doc', zoteroKey: 'ZK99' });
    const s = sources.find((x) => x.source_id === ACFN_FOODWEB_PROMOTION_SOURCE_ID);
    expect(s.url).toBe('https://acfn.com/doc');
  ...
  // the status and report sourceTouched so the write fires.
  it('repairs a stale zotero_status when the key already matches', () => {
    const { records, sources } = makeFixture({}, { zotero_item_key: 'BHHDGZI9', zotero_status: 'pending_owner_export' });
    const applied = applyPromotion(records, sources, { ...APPLY_OPTS, zoteroKey: 'BHHDGZI9' });
    const s = sources.find((x) => x.source_id === ACFN_FOODWEB_PROMOTION_SOURCE_ID);
    expect(s.zotero_status).toBe('linked');
    expect(applied.sourceTouched).toBe(true);
  });
  it('stamps the applicability field on fresh promotion (Evidence Library renders it verbatim)', () => {
    const { records, sources } = makeFixture({
      applicability: 'ACFN WQCIU 2023 community-specific food ingestion rate. needs_review candidate; not a calculator default.',
    });
    applyPromotion(records, sources, APPLY_OPTS);
    const r = records.find((x) => x.parameter_value_id === ACFN_FOODWEB_PROMOTION_VALUE_ID);
  ...
  // Display-stamp REPAIR: an already-approved record missing the applicability stamp is repaired
  // on rerun (an earlier tool version may have stamped uncertainty/review_notes but not applicability).
  it('repairs a missing applicability stamp on an already-approved record (valueTouched=true)', () => {
    const { records, sources } = makeFixture(
      {
        ...DONE_VALUE,
        applicability: 'ACFN WQCIU 2023 community-specific food ingestion rate. needs_review candidate; not a calculator default.',
        uncertainty: 'secondary citation [PROMOTED to approved on 2026-06-14 by J. Nelson]',
        review_notes: 'note [PROMOTED to approved on 2026-06-14 by J. Nelson]',
      },
      { canonical_source_status: 'direct_source_verified' },
    );
  ...
  it('does not double-stamp an already-stamped record (idempotent, valueTouched=false)', () => {
    ...
    const { records, sources } = makeFixture(
      {
        ...DONE_VALUE,
        applicability: 'ACFN WQCIU 2023.' + stamped,
