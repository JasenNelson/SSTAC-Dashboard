// Guards for promote-iris-pfas-dupe-cg-canonical.mjs (promotion of the EPA-designated-primary PFBA/PFHxA
...
import {
  ...
  planPromotion,
  applyPromotion,
} from '../promote-iris-pfas-dupe-cg-canonical.mjs';
...
describe('promote-iris-pfas-dupe-cg-canonical: scope', () => {
  it('targets exactly the 4 EPA-designated-primary PFBA/PFHxA ids', () => {
    expect(IRIS_PFAS_DUPE_CG_CANONICAL_VALUE_IDS).toHaveLength(4);
    expect(new Set(IRIS_PFAS_DUPE_CG_CANONICAL_VALUE_IDS).size).toBe(4);
    for (const id of IRIS_PFAS_DUPE_CG_CANONICAL_VALUE_IDS) expect(id.startsWith('pv-iris-')).toBe(true);
  });
  it('only PFBA + PFHxA in scope (PFDA deferred)', () => {
    for (const r of PROMOTION_ROWS) {
      expect(['perfluorobutanoic_acid_pfba', 'perfluorohexanoic_acid_pfhxa']).toContain(r.substanceKey);
    }
    expect(PROMOTION_ROWS.some((r) => r.substanceKey.includes('pfda'))).toBe(false);
  });
...
describe('promote-iris-pfas-dupe-cg-canonical: parseArgs + validateApplyOptions', () => {
  it('parses flags', () => {
...
describe('promote-iris-pfas-dupe-cg-canonical: planPromotion (fail-closed)', () => {
  it('plans all 4 clean rows', () => {
    const { records, sources } = makeFixture();
    const plan = planPromotion(records, sources, APPLY_OPTS);
    expect(plan.valueResults.filter((vr) => vr.promoteValue)).toHaveLength(4);
...
describe('promote-iris-pfas-dupe-cg-canonical: applyPromotion', () => {
  it('flips qa + canonical + stamps evidence note + value provenance', () => {
    const { records, sources } = makeFixture();
    applyPromotion(records, sources, APPLY_OPTS);
    for (const id of IRIS_PFAS_DUPE_CG_CANONICAL_VALUE_IDS) {
  ...
  it('is idempotent (second apply changes nothing; single stamp)', () => {
    ...
    const r0 = records.find((x) => x.parameter_value_id === IRIS_PFAS_DUPE_CG_CANONICAL_VALUE_IDS[0]);
