// Owner-run promotion helper for the ACFN WQCIU 2023 community-specific food-ingestion-rate record. Plain ASCII only.
//
// WHY THIS EXISTS
// The Phase D ACFN (Athabasca Chipewyan First Nation) food-web scenario uses a community-specific
// ingestion rate from the ACFN Water Quality and Contaminants in the Umsaskis Watershed report
// (WQCIU 2023). The ACFN WQCIU 2023 community-specific rate (0.388 kg/day, 388 g/day) is the
// correct community-specific receptor candidate for the human-health food pathway. This tool
// performs the exact, coupled promotion of EXACTLY the ACFN community-specific VALUE record and
// its ACFN WQCIU SOURCE record, fails closed on any precondition, and is idempotent.
//
// This helper was forked from promote-wlrs-subsistence.mjs (the WLRS subsistence precedent) and
// hardened to match promote-epa-ir-food.mjs: (a) main() now reads the plan RETURNED by
// applyPromotion (so a locator-only re-run actually writes); (b) the source-eligibility precondition
// mirrors classifyCandidate / isDirectCurrentSource (source-role block + direct-current source check
// + exactly-one linked source); plus evidence-state coherence and a display-stamp REPAIR path
// (covers the applicability field, which the Evidence Library renders verbatim).
// NOTE: porting the repair CAPABILITY does not itself change any catalog record. The live ACFN
// food-web record's applicability string is repaired only when the OWNER runs this with --apply
// (its own commit), exactly as the EPA flow landed the owner's --apply diff separately (#294).
//
...
//    ACFN community-specific rate (0.388 kg/day = 388 g/day) against the primary ACFN WQCIU 2023
//    report and file that document in the reference library + Zotero. Running with --apply attests
//    to that verification.
//  - SCOPE (rule 2): only the ACFN community-specific record is promoted. Other ACFN or food-web
//    rows remain as-is. default_status is NEVER modified (stays 'available_option'); the owner's
//    FRAME_DEFAULT_PROFILES row is the activation step, not this tool.
...
//   node scripts/matrix-options/promote-acfn-foodweb.mjs --reviewer "J. Nelson" --date 2026-06-14
//   node scripts/matrix-options/promote-acfn-foodweb.mjs --reviewer "J. Nelson" --date 2026-06-14 --apply
//
...
export const ACFN_FOODWEB_PROMOTION_VALUE_ID = 'pv-acfn-wqciu-2023-ir-food-community-specific';
export const ACFN_FOODWEB_PROMOTION_SOURCE_ID = 'src-acfn-wqciu-2023';
...
export function planPromotion(paramValues, sources, _opts) {
  ...
  const valueRecord = paramValues.find(
    (r) => r.parameter_value_id === ACFN_FOODWEB_PROMOTION_VALUE_ID,
  );
  if (!valueRecord) {
    throw new Error(
  ...
  if (!Array.isArray(valueRecord.evidence_items) || valueRecord.evidence_items.length === 0) {
    throw new Error(
      'Precondition failed: ' + ACFN_FOODWEB_PROMOTION_VALUE_ID + ' has no evidence_items',
    );
  }
  // Fail-closed IDENTITY check: only ever promote the EXACT intended ACFN community-specific
  ...
  // by getFrameSeedCandidateEligibility (notably a wrong jurisdiction is not
  // admitted by the frame), so the promotion would be a silent no-seed.
  ...
  const EXPECTED_VALUE = {
    ...
    candidate_group_id: 'human-health-food__generic__IR_food_kg_per_day__general',
  ...
  if (identityMismatch.length > 0) {
    throw new Error(
      'Precondition failed: ' + ACFN_FOODWEB_PROMOTION_VALUE_ID +
      ...
      identityMismatch
        ...
        .join('\n') +
      '\n(jurisdiction must be "general" -- ensure the record matches the ACFN WQCIU 2023 ' +
      'community-specific food-web rate and the candidate_group_id is correct.)\n' +
      'Refusing to promote a record that cannot seed the intended frame.',
    );
  }
  // source_ids must be EXACTLY [ACFN source]. classifyCandidate derives sourceRoles from EVERY
  ...
  // Requiring exactly one source (the ACFN source) keeps this helper genuinely fail-closed: the one
  ...
  if (!Array.isArray(valueRecord.source_ids) ||
      ...
      valueRecord.source_ids[0] !== ACFN_FOODWEB_PROMOTION_SOURCE_ID) {
    throw new Error(
      'Precondition failed: ' + ACFN_FOODWEB_PROMOTION_VALUE_ID + ' source_ids must be EXACTLY ["' +
      ACFN_FOODWEB_PROMOTION_SOURCE_ID + '"] (actual: ' + JSON.stringify(valueRecord.source_ids) +
  ...
  // Nested provenance must ALSO be the ACFN source (defensive, per codex review 2026-06-14):
  // the top-level source_ids check above does NOT cover evidence_items[*].source_id or
  // source_relationships[*].source_id. A row forked from another food-web record could carry a
  // STALE nested WLRS / reference-mining source_id while its top-level source_ids is correct;
  // applyPromotion preserves those nested ids and stamps QA approved, so a stale nested ref would
  // survive as approved ACFN provenance. Require every nested source reference to be the ACFN source.
  ...
  const staleNestedSourceRefs = nestedSourceRefs.filter(
    (sid) => sid !== ACFN_FOODWEB_PROMOTION_SOURCE_ID,
  );
  if (staleNestedSourceRefs.length > 0) {
    throw new Error(
      'Precondition failed: ' + ACFN_FOODWEB_PROMOTION_VALUE_ID + ' has nested provenance source ' +
      'reference(s) that are not the ACFN source (' + JSON.stringify(staleNestedSourceRefs) + '). ' +
      'Every evidence_items[*].source_id and source_relationships[*].source_id must be "' +
      ACFN_FOODWEB_PROMOTION_SOURCE_ID + '"; a stale forked WLRS / reference-mining nested source ' +
      'would otherwise survive as approved ACFN provenance. Refusing to promote.',
    );
  }
  ...
  const sourceRecord = sources.find(
    (s) => s.source_id === ACFN_FOODWEB_PROMOTION_SOURCE_ID,
  );
  if (!sourceRecord) {
    throw new Error(
      'Precondition failed: source record not found in sources.json: ' +
      ACFN_FOODWEB_PROMOTION_SOURCE_ID,
    );
  }
  ...
  if (!sourceAlreadyDone && !sourceExpectedPre) {
    throw new Error(
      'Precondition failed: source ' + ACFN_FOODWEB_PROMOTION_SOURCE_ID +
  ...
  if (allRoles.includes('policy_compilation') || allRoles.includes('reference_mining')) {
    throw new Error(
      'Precondition failed: ' + ACFN_FOODWEB_PROMOTION_VALUE_ID + ' has a source role of ' +
...
const BANNER = [
  '=============================================================================',
  ' promote-acfn-foodweb.mjs -- owner-run ACFN WQCIU community-specific IR-food promotion',
  '=============================================================================',
  '',
  'OWNER VERIFICATION REQUIRED before --apply:',
  '  Verify the ACFN community-specific rate (0.388 kg/day = 388 g/day) against the',
  '  primary ACFN WQCIU 2023 report. File the document in the reference library + Zotero',
  '  before running --apply. Running --apply attests to that verification.',
  '',
  'DURABLE LOCATOR:',
  '  Provide --source-url and/or --zotero-key to satisfy the provenance guard.',
  '  Optional provenance strengthening:',
  '    --zotero-key "<Zotero item key>"  link the Zotero record when you file the document',
  '    --source-url "<url>"              set or override the stored URL',
  '  (Fail-closed guard: --apply fails ONLY if the source row has NEITHER url nor key.)',
  '',
  'SCOPE: only the ACFN community-specific value + its ACFN WQCIU source are promoted.',
  '  Other ACFN or food-web rows are not touched.',
...
const HELP = [
  'promote-acfn-foodweb.mjs -- owner-run ACFN WQCIU community-specific IR-food promotion tool.',
  '',
  'Usage:',
  '  node scripts/matrix-options/promote-acfn-foodweb.mjs --reviewer "<id>" --date YYYY-MM-DD [--apply]',
  ...
  '  --source-url "<url>"     URL to the primary document.',
  '                           Set on the source row when --apply writes sources.json.',
  '  --zotero-key "<key>"     Zotero item key (e.g. BHHDGZI9; right-click item in Zotero).',
  ...
  '',
  'Targets:',
  '  VALUE : pv-acfn-wqciu-2023-ir-food-community-specific   (parameter_values.json)',
  '  SOURCE: src-acfn-wqciu-2023                              (sources.json)',
  '',
...
function printPlan(plan, opts) {
  if (plan.promoteValue) {
    const r = plan.valueRecord;
    console.log('  VALUE   PROMOTE  ' + ACFN_FOODWEB_PROMOTION_VALUE_ID);
    ...
    console.log('  VALUE   SKIP     ' + ACFN_FOODWEB_PROMOTION_VALUE_ID + ': already in target state (no-op)');
  }

  if (plan.promoteSource) {
    const s = plan.sourceRecord;
    console.log('  SOURCE  PROMOTE  ' + ACFN_FOODWEB_PROMOTION_SOURCE_ID);
    ...
    console.log('  SOURCE  SKIP     ' + ACFN_FOODWEB_PROMOTION_SOURCE_ID +
...
function main() {
  ...
  if (valueWillBeApproved && sourceWillBeVerified && !effUrl && !effKey) {
    throw new Error(
      'Provenance guard: ' + ACFN_FOODWEB_PROMOTION_VALUE_ID + ' would be approved against source ' +
      ACFN_FOODWEB_PROMOTION_SOURCE_ID + ' (direct_source_verified), but that source has no durable' +
      ' locator (url=null, zotero_item_key=null).\n' +
      'Provide --source-url "<url>" and/or --zotero-key "<Zotero item key>"' +
      ' (set when you file the document in Zotero) so the primary document is reproducibly locatable.',
    );
  }
