// Owner-run promotion helper for the HC PQRA v4.0 residential-toddler direct-contact receptor
// profile. Plain ASCII only.
//
// WHY THIS EXISTS
// The human-health direct-contact pathway frame for the residential-toddler receptor requires 7
...
// This tool promotes EXACTLY those 7 records + the source, fails closed on any precondition, and
// is idempotent.
//
// It mirrors promote-wlrs-bw-default.mjs in structure, style, and fail-closed discipline, adapted
// for a multi-record batch promotion.
//
...
//    7 values against the primary Health Canada PQRA v4.0 PDF (Cat. H129-114/2023E-PDF, ISBN
//    978-0-660-68497-0, March 2024). Running with --apply attests to that verification.
//  - SCOPE (rule 2): only the 7 listed records are promoted. default_status is NEVER modified
//    (stays 'available_option'); the owner's FRAME_DEFAULT_PROFILES row is the activation step.
//  - SOURCE (rule 3): src-health-canada-pqra-v4-2024 does NOT currently carry canonical_source_status.
//    This tool ADDS that field (= 'direct_source_verified') on --apply. This is REQUIRED because
//    defaultSelectionPolicy.isDirectCurrentSource() checks source.canonical_source_status ===
//    'direct_source_verified'. Without it the frame seed stays blocked even after value promotion.
...
//   node scripts/matrix-options/promote-hc-pqra-direct.mjs --reviewer "J. Nelson" --date 2026-06-12
//   node scripts/matrix-options/promote-hc-pqra-direct.mjs --reviewer "J. Nelson" --date 2026-06-12 --apply
//
...
export const HC_PQRA_PROMOTION_VALUE_IDS = [
  'pv-hc-pqra-v4-2024-bw-toddler-ca',
  'pv-hc-pqra-v4-2024-ir-sed-toddler-ca',
  'pv-hc-pqra-v4-2024-ef-residential-ca',
  'pv-hc-pqra-v4-2024-ed-residential-ca',
  'pv-hc-pqra-v4-2024-at-cancer-lifetime-ca',
  'pv-hc-pqra-v4-2024-sa-total-toddler-ca',
  'pv-hc-pqra-v4-2024-af-sed-other-general-ca',
];
export const HC_PQRA_PROMOTION_SOURCE_ID = 'src-health-canada-pqra-v4-2024';

// Expected frame-eligible identity per record (fields classifyCandidate / getFrameSeedCandidateEligibility
// examine). Any mismatch -> fail closed. Ordered to match HC_PQRA_PROMOTION_VALUE_IDS above.
...
function planOneValueRecord(paramValues, valueId, expectedIdentity) {
  ...
  if (!Array.isArray(valueRecord.source_ids) ||
      ...
      valueRecord.source_ids[0] !== HC_PQRA_PROMOTION_SOURCE_ID) {
    throw new Error(
      'Precondition failed: ' + valueId + ' source_ids must be EXACTLY ["' +
      HC_PQRA_PROMOTION_SOURCE_ID + '"] (actual: ' + JSON.stringify(valueRecord.source_ids) +
  ...
  const staleNestedSourceRefs = nestedSourceRefs.filter(
    (sid) => sid !== HC_PQRA_PROMOTION_SOURCE_ID,
  );
  if (staleNestedSourceRefs.length > 0) {
    throw new Error(
      ...
      HC_PQRA_PROMOTION_SOURCE_ID + '"; a stale nested source reference ' +
...
// Computes the promotion plan for all 7 value records + the source. Enforces ALL preconditions
...
export function planPromotion(paramValues, sources, _opts) {
  ...
  const valueResults = HC_PQRA_PROMOTION_VALUE_IDS.map((valueId, i) =>
    planOneValueRecord(paramValues, valueId, EXPECTED_IDENTITIES[i]),
  );

  // -- SOURCE record --
  const sourceRecord = sources.find(
    (s) => s.source_id === HC_PQRA_PROMOTION_SOURCE_ID,
  );
  if (!sourceRecord) {
    throw new Error(
  ...
  if (!sourceAlreadyDone && !sourceExpectedPre) {
    throw new Error(
      'Precondition failed: source ' + HC_PQRA_PROMOTION_SOURCE_ID +
...
const BANNER = [
  '=========================================================================',
  ' promote-hc-pqra-direct.mjs -- owner-run HC PQRA v4.0 direct-contact promotion',
  '=========================================================================',
  '',
  'OWNER VERIFICATION REQUIRED before --apply:',
  '  Verify all 7 values against the primary Health Canada PQRA v4.0 PDF:',
  '    Cat. H129-114/2023E-PDF, ISBN 978-0-660-68497-0, March 2024.',
  '  Running --apply attests to that verification.',
  '',
  'SOURCE NOTE:',
  '  src-health-canada-pqra-v4-2024 does NOT currently carry canonical_source_status.',
  '  This tool ADDS canonical_source_status = direct_source_verified on --apply.',
  '  This is required for defaultSelectionPolicy.isDirectCurrentSource() to pass.',
  '',
  'SCOPE: 7 direct-contact receptor value records + 1 source are promoted. default_status',
...
const HELP = [
  'promote-hc-pqra-direct.mjs -- owner-run HC PQRA v4.0 residential-toddler profile promotion.',
  '',
  'Usage:',
  '  node scripts/matrix-options/promote-hc-pqra-direct.mjs --reviewer "<id>" --date YYYY-MM-DD [--apply]',
  ...
  '',
  'Targets (7 VALUE records + 1 SOURCE):',
  '  VALUE : pv-hc-pqra-v4-2024-bw-toddler-ca',
  '  VALUE : pv-hc-pqra-v4-2024-ir-sed-toddler-ca',
  '  VALUE : pv-hc-pqra-v4-2024-ef-residential-ca',
  '  VALUE : pv-hc-pqra-v4-2024-ed-residential-ca',
  '  VALUE : pv-hc-pqra-v4-2024-at-cancer-lifetime-ca',
  '  VALUE : pv-hc-pqra-v4-2024-sa-total-toddler-ca',
  '  VALUE : pv-hc-pqra-v4-2024-af-sed-other-general-ca',
  ...
  '  canonical_source_status   (absent) -> direct_source_verified  [FIELD ADDED]',
  '',
...
function printPlan(plan, opts) {
  for (let i = 0; i < plan.valueResults.length; i++) {
    ...
    const valueId = HC_PQRA_PROMOTION_VALUE_IDS[i];
    if (vr.promoteValue) {
      const r = vr.valueRecord;
  ...
  if (plan.promoteSource) {
    ...
    console.log('  SOURCE  PROMOTE  ' + HC_PQRA_PROMOTION_SOURCE_ID);
    console.log('    canonical_source_status : ' + currentStatus +
      ' -> direct_source_verified');
  } else {
    console.log('  SOURCE  SKIP     ' + HC_PQRA_PROMOTION_SOURCE_ID +
...
function main() {
  ...
  if (anyValueWillBeApproved && sourceWillBeVerified && !effUrl && !effKey) {
    throw new Error(
      ...
      HC_PQRA_PROMOTION_SOURCE_ID + ' (direct_source_verified), but that source has no durable' +
