// Owner-run promotion helper for the HC PQRA v4.0 residential-adult direct-contact receptor
// profile. Plain ASCII only.
//
// WHY THIS EXISTS
// The human-health direct-contact pathway frame for the residential-adult receptor requires 3
...
// This tool promotes EXACTLY those 3 records + the source, fails closed on any precondition, and
// is idempotent.
//
...
//    3 values against the primary Health Canada PQRA v4.0 PDF (Cat. H129-114/2023E-PDF, ISBN
//    978-0-660-68497-0, March 2024). Running with --apply attests to that verification.
//  - SCOPE (rule 2): only the 3 listed records are promoted. default_status is NEVER modified
...
//    canonical_source_status = 'direct_source_verified' (promoted by the toddler run, #305).
...
//   node scripts/matrix-options/promote-hc-pqra-adult.mjs --reviewer "J. Nelson" --date 2026-06-12
//   node scripts/matrix-options/promote-hc-pqra-adult.mjs --reviewer "J. Nelson" --date 2026-06-12 --apply
//
...
export const HC_PQRA_ADULT_PROMOTION_VALUE_IDS = [
  'pv-hc-pqra-v4-2024-bw-adult-ca',
  'pv-hc-pqra-v4-2024-ir-sed-general-ca',
  'pv-hc-pqra-v4-2024-sa-total-adult-ca',
];
export const HC_PQRA_ADULT_PROMOTION_SOURCE_ID = 'src-health-canada-pqra-v4-2024';

// Expected frame-eligible identity per record (fields classifyCandidate / getFrameSeedCandidateEligibility
// examine). Any mismatch -> fail closed. Ordered to match HC_PQRA_ADULT_PROMOTION_VALUE_IDS above.
...
function planOneValueRecord(paramValues, valueId, expectedIdentity) {
  ...
  if (!Array.isArray(valueRecord.source_ids) ||
      ...
      valueRecord.source_ids[0] !== HC_PQRA_ADULT_PROMOTION_SOURCE_ID) {
    throw new Error(
      'Precondition failed: ' + valueId + ' source_ids must be EXACTLY ["' +
      HC_PQRA_ADULT_PROMOTION_SOURCE_ID + '"] (actual: ' + JSON.stringify(valueRecord.source_ids) +
  ...
  const staleNestedSourceRefs = nestedSourceRefs.filter(
    (sid) => sid !== HC_PQRA_ADULT_PROMOTION_SOURCE_ID,
  );
  if (staleNestedSourceRefs.length > 0) {
    throw new Error(
      ...
      HC_PQRA_ADULT_PROMOTION_SOURCE_ID + '"; a stale nested source reference ' +
...
// Computes the promotion plan for all 3 value records + the source. Enforces ALL preconditions
...
export function planPromotion(paramValues, sources, _opts) {
  ...
  const valueResults = HC_PQRA_ADULT_PROMOTION_VALUE_IDS.map((valueId, i) =>
    planOneValueRecord(paramValues, valueId, EXPECTED_IDENTITIES[i]),
  );

  // -- SOURCE record --
  const sourceRecord = sources.find(
    (s) => s.source_id === HC_PQRA_ADULT_PROMOTION_SOURCE_ID,
  );
  if (!sourceRecord) {
    throw new Error(
  ...
  if (!sourceAlreadyDone && !sourceExpectedPre) {
    throw new Error(
      'Precondition failed: source ' + HC_PQRA_ADULT_PROMOTION_SOURCE_ID +
...
const BANNER = [
  '=========================================================================',
  ' promote-hc-pqra-adult.mjs -- owner-run HC PQRA v4.0 adult-receptor promotion',
  '=========================================================================',
  '',
  'OWNER VERIFICATION REQUIRED before --apply:',
  '  Verify the 3 adult-receptor values against the primary Health Canada PQRA v4.0 PDF',
  ...
  '',
  'SCOPE: 3 adult-receptor value records + 1 source are promoted. default_status',
...
const HELP = [
  'promote-hc-pqra-adult.mjs -- owner-run HC PQRA v4.0 residential-adult profile promotion.',
  '',
  'Usage:',
  '  node scripts/matrix-options/promote-hc-pqra-adult.mjs --reviewer "<id>" --date YYYY-MM-DD [--apply]',
  ...
  '',
  'Targets (3 adult-receptor VALUE records + 1 SOURCE):',
  '  VALUE : pv-hc-pqra-v4-2024-bw-adult-ca',
  '  VALUE : pv-hc-pqra-v4-2024-ir-sed-general-ca',
  '  VALUE : pv-hc-pqra-v4-2024-sa-total-adult-ca',
...
function printPlan(plan, opts) {
  for (let i = 0; i < plan.valueResults.length; i++) {
    ...
    const valueId = HC_PQRA_ADULT_PROMOTION_VALUE_IDS[i];
    if (vr.promoteValue) {
      const r = vr.valueRecord;
  ...
  if (plan.promoteSource) {
    ...
    console.log('  SOURCE  PROMOTE  ' + HC_PQRA_ADULT_PROMOTION_SOURCE_ID);
    console.log('    canonical_source_status : ' + currentStatus +
      ' -> direct_source_verified');
  } else {
    console.log('  SOURCE  SKIP     ' + HC_PQRA_ADULT_PROMOTION_SOURCE_ID +
...
function main() {
  ...
  if (anyValueWillBeApproved && sourceWillBeVerified && !effUrl && !effKey) {
    throw new Error(
      ...
      HC_PQRA_ADULT_PROMOTION_SOURCE_ID + ' (direct_source_verified), but that source has no durable' +
