// Owner-run promotion helper for the BC WLRS low-level fish-ingestion-rate record. Plain ASCII only.
//
// WHY THIS EXISTS
// The human-health food pathway BC frame offers several BC WLRS 2023 fish-ingestion-rate receptor
// candidates (recreational [approved], subsistence, and this low-level rate). The BC WLRS 2023
// low-level fish ingestion rate (0.021 kg/day, 21 g/day) is the lowest-consumption candidate for the
// human-health food pathway. This tool performs the exact, coupled promotion of EXACTLY the low-level
// VALUE record and its WLRS SOURCE record, fails closed on any precondition, and is idempotent.
//
// This helper was forked from promote-wlrs-subsistence.mjs (its sibling receptor) and retains the
// same fail-closed discipline matched to promote-epa-ir-food.mjs: (a) main() reads the plan RETURNED
// by applyPromotion (so a locator-only re-run actually writes); (b) the source-eligibility
// precondition mirrors classifyCandidate / isDirectCurrentSource (source-role block + direct-current
// source check + exactly-one linked source); plus evidence-state coherence and a display-stamp REPAIR
// path (covers the applicability field, which the Evidence Library renders verbatim).
// NOTE: porting the repair CAPABILITY does not itself change any catalog record. The live WLRS
// low-level record's applicability string is repaired only when the OWNER runs this with --apply.
//
...
//    WLRS low-level rate (0.021 kg/day = 21 g/day) against the primary BC WLRS 2023 PDF
//    ("Derivation of Screening Values for Contaminants in Fish Tissue", ISBN 978-1-0399-0019-6) and
//    file that PDF in the reference library + Zotero. Running with --apply attests to that verification.
//  - SCOPE (rule 2): only the low-level-fisher record is promoted. The recreational (already approved)
//    and subsistence WLRS rows remain as-is.
...
//   node scripts/matrix-options/promote-wlrs-low-level.mjs --reviewer "J. Nelson" --date 2026-06-20
//   node scripts/matrix-options/promote-wlrs-low-level.mjs --reviewer "J. Nelson" --date 2026-06-20 --apply
//
...
export const WLRS_LOW_LEVEL_PROMOTION_VALUE_ID = 'pv-wlrs-2023-ir-food-low-level-bc';
export const WLRS_LOW_LEVEL_PROMOTION_SOURCE_ID = 'src-bc-wlrs-fish-tissue-screening-2023';
...
export function planPromotion(paramValues, sources, _opts) {
  ...
  const valueRecord = paramValues.find(
    (r) => r.parameter_value_id === WLRS_LOW_LEVEL_PROMOTION_VALUE_ID,
  );
  if (!valueRecord) {
    throw new Error(
  ...
  if (!Array.isArray(valueRecord.evidence_items) || valueRecord.evidence_items.length === 0) {
    throw new Error(
      'Precondition failed: ' + WLRS_LOW_LEVEL_PROMOTION_VALUE_ID + ' has no evidence_items',
    );
  }
  // Fail-closed IDENTITY check: only ever promote the EXACT intended WLRS low-level record in its
  // frame-eligible shape. Promoting the status fields on a record whose jurisdiction/candidate_group/
  // value/unit are wrong would "succeed" yet still be BLOCKED by getFrameSeedCandidateEligibility, so
  // the promotion would be a silent no-seed. Cover EVERY scalar field that
  // getFrameSeedCandidateEligibility / classifyCandidate examine.
  ...
  if (identityMismatch.length > 0) {
    throw new Error(
      'Precondition failed: ' + WLRS_LOW_LEVEL_PROMOTION_VALUE_ID +
      ...
      'bc-protocol1-v5-dra frame and could not seed C-BC.)\n' +
  ...
  if (!Array.isArray(valueRecord.source_ids) ||
      ...
      valueRecord.source_ids[0] !== WLRS_LOW_LEVEL_PROMOTION_SOURCE_ID) {
    throw new Error(
      'Precondition failed: ' + WLRS_LOW_LEVEL_PROMOTION_VALUE_ID + ' source_ids must be EXACTLY ["' +
      WLRS_LOW_LEVEL_PROMOTION_SOURCE_ID + '"] (actual: ' + JSON.stringify(valueRecord.source_ids) +
  ...
  // survive as approved provenance. Require every nested source reference to be the WLRS source.
  ...
  const staleNestedSourceRefs = nestedSourceRefs.filter(
    (sid) => sid !== WLRS_LOW_LEVEL_PROMOTION_SOURCE_ID,
  );
  if (staleNestedSourceRefs.length > 0) {
    throw new Error(
      'Precondition failed: ' + WLRS_LOW_LEVEL_PROMOTION_VALUE_ID + ' has nested provenance source ' +
      ...
      WLRS_LOW_LEVEL_PROMOTION_SOURCE_ID + '"; a stale nested source reference ' +
  ...
  // state. The evidence items must move WITH the top-level statuses. A record whose top-level statuses
  // are promoted but whose evidence_items still hold needs_review (or vice versa) is a partially-promoted
  // DRIFT: classifying it as already-done would skip approveEvidence() and leave the Evidence Library
  // rendering stale evidence QA. Attestation guard (uniform backport 2026-06-13): an approved evidence
  // item MUST carry the owner attestation (reviewed_by + reviewed_at); an approved-but-unattested item
  // is NOT counted approved here, so the record fails closed (drift) rather than being skipped.
  ...
  const sourceRecord = sources.find(
    (s) => s.source_id === WLRS_LOW_LEVEL_PROMOTION_SOURCE_ID,
  );
  if (!sourceRecord) {
    throw new Error(
      'Precondition failed: source record not found in sources.json: ' +
      WLRS_LOW_LEVEL_PROMOTION_SOURCE_ID,
    );
  }
  ...
  if (!sourceAlreadyDone && !sourceExpectedPre) {
    throw new Error(
      'Precondition failed: source ' + WLRS_LOW_LEVEL_PROMOTION_SOURCE_ID +
  ...
  if (allRoles.includes('policy_compilation') || allRoles.includes('reference_mining')) {
    throw new Error(
      'Precondition failed: ' + WLRS_LOW_LEVEL_PROMOTION_VALUE_ID + ' has a source role of ' +
...
// rendered verbatim by EvidenceLibrary; uncertainty / review_notes likewise).
...
// and as a display-stamp REPAIR for an already-approved record that is missing a stamp.
...
export function applyPromotion(paramValues, sources, opts) {
  ...
  if (plan.promoteValue) {
    ...
    // Stamp the human-readable provenance fields so the Evidence Library, which renders them verbatim,
    // does not show stale "pending / needs_review" text on a now-approved record.
    ...
    // earlier tool version did not cover. Append idempotently so an approved row never renders a stale
    // needs_review label. valueTouched drives the param_values write in main().
  ...
  // already-verified-but-unlocatable source can still be made locatable.
...
const BANNER = [
  '=============================================================================',
  ' promote-wlrs-low-level.mjs -- owner-run WLRS low-level IR-food promotion',
  '=============================================================================',
  '',
  'OWNER VERIFICATION REQUIRED before --apply:',
  '  Verify the WLRS low-level rate (0.021 kg/day = 21 g/day) against the primary',
  '  BC WLRS 2023 PDF (ISBN 978-1-0399-0019-6, fish-tissue screening derivation).',
  '  File the PDF in the reference library + Zotero before running --apply.',
  ...
  '',
  'SCOPE: only the low-level-fisher value + its WLRS source are promoted.',
  '  The recreational (already approved) and subsistence WLRS rows are not touched.',
...
const HELP = [
  'promote-wlrs-low-level.mjs -- owner-run BC WLRS low-level IR-food promotion tool.',
  '',
  'Usage:',
  '  node scripts/matrix-options/promote-wlrs-low-level.mjs --reviewer "<id>" --date YYYY-MM-DD [--apply]',
  ...
  '  of a locator. --source-url / --zotero-key are OPTIONAL.',
  '  (The guard fails --apply ONLY when the source row has NEITHER url nor zotero key.)',
  '',
  'Targets:',
  '  VALUE : pv-wlrs-2023-ir-food-low-level-bc      (parameter_values.json)',
...
function printPlan(plan, opts) {
  if (plan.promoteValue) {
    const r = plan.valueRecord;
    console.log('  VALUE   PROMOTE  ' + WLRS_LOW_LEVEL_PROMOTION_VALUE_ID);
    ...
    console.log('  VALUE   SKIP     ' + WLRS_LOW_LEVEL_PROMOTION_VALUE_ID + ': already in target state (no-op)');
  }

  if (plan.promoteSource) {
    const s = plan.sourceRecord;
    console.log('  SOURCE  PROMOTE  ' + WLRS_LOW_LEVEL_PROMOTION_SOURCE_ID);
    ...
    console.log('  SOURCE  SKIP     ' + WLRS_LOW_LEVEL_PROMOTION_SOURCE_ID +
...
function main() {
  ...
  // promoted OR was ALREADY verified. Effective locator accounts for --source-url/--zotero-key.
  ...
  if (valueWillBeApproved && sourceWillBeVerified && !effUrl && !effKey) {
    throw new Error(
      'Provenance guard: ' + WLRS_LOW_LEVEL_PROMOTION_VALUE_ID + ' would be approved against source ' +
      WLRS_LOW_LEVEL_PROMOTION_SOURCE_ID + ' (direct_source_verified), but that source has no durable' +
  ...
  // Includes a Zotero status-only repair (key already set but zotero_status stale).
  ...
  // A display-stamp repair (already-approved value record missing a provenance stamp) is also work.
