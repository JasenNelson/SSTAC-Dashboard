// Owner-run promotion helper for the US EPA general-population fish-ingestion-rate default seed.
// Plain ASCII only.
//
// WHY THIS EXISTS
// The Phase C "C-nonBC" cross-frame smoke test seeds the us-epa-usace-sediment frame's human-health
// food pathway (IR_food_kg_per_day) from an approved, direct-source-verified catalog record. The US
// EPA 2000 AWQC general adult population fish consumption rate (17.5 g/day = 0.0175 kg/day) is the
// natural non-BC candidate (a clean cross-frame delta vs the BC WLRS 0.111 recreational rate). This
// tool performs the exact, coupled promotion of EXACTLY the EPA general-population VALUE record and
// its EPA 2000 AWQC SOURCE record, fails closed on any precondition, and is idempotent. It mirrors
// promote-wlrs-default.mjs (the C-BC precedent) re-scoped to the EPA records.
//
...
//    EPA general adult population rate (0.0175 kg/day) against the primary US EPA 2000 AWQC Human
//    Health Methodology (EPA-822-B-00-004) and file that PDF in the reference library + Zotero.
//    Running with --apply attests to that verification.
//  - SCOPE (rule 2): only the EPA general-population record is promoted as the US-frame default
//    candidate. default_status is NEVER modified (stays 'available_option'); the owner's
...
//   node scripts/matrix-options/promote-epa-ir-food.mjs --reviewer "J. Nelson" --date 2026-06-10
//   node scripts/matrix-options/promote-epa-ir-food.mjs --reviewer "J. Nelson" --date 2026-06-10 \
//     --source-url "https://www.epa.gov/..." --apply
//
...
export const EPA_PROMOTION_VALUE_ID = 'pv-epa-2000-ir-food-general-us';
export const EPA_PROMOTION_SOURCE_ID = 'src-epa-2000-awqc-human-health';
...
export function planPromotion(paramValues, sources, _opts) {
  ...
  const valueRecord = paramValues.find(
    (r) => r.parameter_value_id === EPA_PROMOTION_VALUE_ID,
  );
  if (!valueRecord) {
    throw new Error(
  ...
  if (!Array.isArray(valueRecord.evidence_items) || valueRecord.evidence_items.length === 0) {
    throw new Error(
      'Precondition failed: ' + EPA_PROMOTION_VALUE_ID + ' has no evidence_items',
    );
  }
  // Fail-closed IDENTITY check: only ever promote the EXACT intended EPA general-population record
  // in its frame-eligible shape. Promoting the status fields on a record whose
  // jurisdiction/candidate_group/value/unit are wrong would "succeed" yet still be BLOCKED by
  // getFrameSeedCandidateEligibility (notably a jurisdiction the us-epa-usace-sediment frame does
  // not admit would be a silent no-seed). Cover EVERY scalar field that
  // getFrameSeedCandidateEligibility / classifyCandidate examine, so the helper cannot "succeed"
  // on a record that would still be blocked.
  ...
  if (identityMismatch.length > 0) {
    throw new Error(
      'Precondition failed: ' + EPA_PROMOTION_VALUE_ID +
      ...
      identityMismatch
        ...
        .join('\n') +
      '\n(jurisdiction must be "US_federal" -- the us-epa-usace-sediment frame admits US_federal; a ' +
      'row in any other jurisdiction could not seed C-nonBC.)\n' +
      'Refusing to promote a record that cannot seed the US EPA frame.',
    );
  }
  // source_ids must be EXACTLY [EPA source]. classifyCandidate derives sourceRoles from EVERY
  ...
  // would be blocked by the real frame-default pipeline yet slip past the single-source check below.
  // Requiring exactly one source (the EPA source) keeps this helper genuinely fail-closed: the one
  ...
  if (!Array.isArray(valueRecord.source_ids) ||
      ...
      valueRecord.source_ids[0] !== EPA_PROMOTION_SOURCE_ID) {
    throw new Error(
      'Precondition failed: ' + EPA_PROMOTION_VALUE_ID + ' source_ids must be EXACTLY ["' +
      EPA_PROMOTION_SOURCE_ID + '"] (actual: ' + JSON.stringify(valueRecord.source_ids) +
  ...
  const sourceRecord = sources.find(
    (s) => s.source_id === EPA_PROMOTION_SOURCE_ID,
  );
  if (!sourceRecord) {
    throw new Error(
  ...
  if (!sourceAlreadyDone && !sourceExpectedPre) {
    throw new Error(
      'Precondition failed: source ' + EPA_PROMOTION_SOURCE_ID +
  ...
  if (allRoles.includes('policy_compilation') || allRoles.includes('reference_mining')) {
    throw new Error(
      'Precondition failed: ' + EPA_PROMOTION_VALUE_ID + ' has a source role of ' +
...
const BANNER = [
  '=========================================================================',
  ' promote-epa-ir-food.mjs -- owner-run US EPA fish-ingestion-rate promotion',
  '=========================================================================',
  '',
  'OWNER VERIFICATION REQUIRED before --apply:',
  '  Verify the US EPA general adult population rate (0.0175 kg/day = 17.5 g/day)',
  '  against the primary US EPA 2000 AWQC Human Health Methodology (EPA-822-B-00-004),',
  '  general-population adult fish ingestion rate. File the PDF in the reference',
  '  library + Zotero before running --apply. Running --apply attests to that verification.',
  '',
  ...
  '    --source-url "<epa.gov URL>"      e.g. the EPA-822-B-00-004 PDF download URL',
  ...
  '',
  'SCOPE: only the EPA general-population value + its EPA 2000 AWQC source are promoted.',
...
const HELP = [
  'promote-epa-ir-food.mjs -- owner-run US EPA fish-ingestion-rate promotion tool.',
  '',
  'Usage:',
  '  node scripts/matrix-options/promote-epa-ir-food.mjs --reviewer "<id>" --date YYYY-MM-DD [--apply]',
  ...
  '  --source-url "<url>"     URL to the primary PDF (e.g. epa.gov download link).',
  ...
  '',
  'Targets:',
  '  VALUE : pv-epa-2000-ir-food-general-us  (parameter_values.json)',
  '  SOURCE: src-epa-2000-awqc-human-health  (sources.json)',
  '',
...
function printPlan(plan, opts) {
  if (plan.promoteValue) {
    const r = plan.valueRecord;
    console.log('  VALUE   PROMOTE  ' + EPA_PROMOTION_VALUE_ID);
    ...
    console.log('  VALUE   SKIP     ' + EPA_PROMOTION_VALUE_ID + ': already in target state (no-op)');
  }

  if (plan.promoteSource) {
    const s = plan.sourceRecord;
    console.log('  SOURCE  PROMOTE  ' + EPA_PROMOTION_SOURCE_ID);
    ...
    console.log('  SOURCE  SKIP     ' + EPA_PROMOTION_SOURCE_ID +
...
function main() {
  ...
  if (valueWillBeApproved && sourceWillBeVerified && !effUrl && !effKey) {
    throw new Error(
      'Provenance guard: ' + EPA_PROMOTION_VALUE_ID + ' would be approved against source ' +
      EPA_PROMOTION_SOURCE_ID + ' (direct_source_verified), but that source has no durable' +
  ...
  // object that carries sourceTouched (the locator-only write signal). main's outer `plan` never
  // gets sourceTouched, so the write decision MUST read the returned object -- otherwise a
  // locator-only update (both records already promoted, only the url/zotero key changing) mutates
  // the in-memory records but never writes the file.
