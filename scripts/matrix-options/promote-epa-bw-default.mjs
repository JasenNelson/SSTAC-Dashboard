...
export function planPromotion(paramValues, sources, _opts) {
  ...
  const staleNestedSourceRefs = nestedSourceRefs.filter(
    (sid) => sid !== EPA_BW_PROMOTION_SOURCE_ID,
  );
  if (staleNestedSourceRefs.length > 0) {
    throw new Error(
      'Precondition failed: ' + EPA_BW_PROMOTION_VALUE_ID + ' has nested provenance source ' +
      ...
      EPA_BW_PROMOTION_SOURCE_ID + '"; a stale nested source reference ' +
