...
export function planPromotion(paramValues, sources, _opts) {
  ...
  const staleNestedSourceRefs = nestedSourceRefs.filter(
    (sid) => sid !== WLRS_PROMOTION_SOURCE_ID,
  );
  if (staleNestedSourceRefs.length > 0) {
    throw new Error(
      'Precondition failed: ' + WLRS_PROMOTION_VALUE_ID + ' has nested provenance source ' +
      ...
      WLRS_PROMOTION_SOURCE_ID + '"; a stale nested source reference ' +
