-- Backfill migration: v2_projects, v2_submission_files, v2_extraction_runs,
-- enforce_project_caps_v2() trigger, their RLS policies, storage.objects RLS for the
-- v2-submissions bucket, and table GRANTs. (Gate 2B branch-replay fix, Tier 1, file 1 of 6.)
--
-- Verbatim copy of supabase/engine_v2/database_schema_engine_v2_patch.sql. That file was hand-pasted
-- into Supabase Studio's SQL Editor and never registered as a tracked migration -- see
-- supabase/migrations/20260510000001_user_roles_backfill_for_branch_replay.sql for the full
-- untracked-schema rationale (same pattern). Timestamped 20260512000001 (after Tier 0's
-- 20260510/20260511, before 20260513) purely for branch-replay dependency order; not a claim about
-- real historical apply dates. Depends on: `user_roles` (Tier 0, 20260510000001) for the RLS policies
-- below that check `EXISTS (SELECT 1 FROM user_roles WHERE ...)`; `auth.users` (Supabase default).
-- Also creates storage.objects RLS policies scoped to a `v2-submissions` bucket -- the bucket ROW
-- itself is a separate Supabase Storage runtime precondition not created by any SQL in this repo
-- (does not block DDL replay, only real file-upload functionality; a known, accepted, non-blocking
-- gap).
--
...
-- (no canonical/tracked DDL source exists for it in this repo; see
-- docs/archive/root-scratch-2026/DEMO_BLOCKER_RESOLUTION_KIT_2026_06_04.md "Remedy G" for an
-- archived, code-derived, unverified reconstruction).
--
